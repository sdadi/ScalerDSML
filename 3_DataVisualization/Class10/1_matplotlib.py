...
print(data.head())
print('len is rows count, and data.size is total elements in complete data set rows*columns\n',len(data),data.size,data.shape)
...
print('unique genre from the dataset is ', data['Genre'].unique())
print('count of unique Genre is: ',data['Genre'].nunique())
...
#bar chart
# plt.bar(x_val,y_val)
plt.title('Bar chart: genre vs count')
plt.xlabel('genre')
plt.ylabel('count')
# plt.show()
...
plt.bar(x_val,y_val)
plt.title('Bar chart with ticks: genre vs count')
plt.xlabel('genre')
plt.ylabel('count')
...
plt.title('sea born title: genre vs counts')
names = data['Genre'].value_counts().index
names = names.sort_values()
print('sorted names for seaborn count plot:',names)
...
plt.title('Box plot of Global_Sales')
